...
void ANOVA::SetColumnNames(vector<string> names){
    nameOfColumns.clear();
    for(auto x:names){
        nameOfColumns.push_back(x);
    }
}
...
int ANOVA::RepeatOfLevel(int col, double v){
    int C = ExpData.size();
    int rs = 0;
    for(int c=0;c<C;c++){
        double xv = ExpData.at(c).x.at(col);
...
double ANOVA::SumSqrOf(int col){
    vector<double> vs = LevelsOfCol(col);
    int L= vs.size();

    double yBar = averageY();

    double sumsq=0.0;
    for(int j=0;j<L;j++){
           double levelsum = SumOfYatLevel(col,vs.at(j));
           int repeat = RepeatOfLevel(col,vs.at(j));
           double levelAverage = levelsum/repeat;
           sumsq += (repeat * (levelAverage - yBar)*(levelAverage - yBar) );
    }
...
void ANOVA::ExportToMatlabScript(string mfile){
    ofstream res;
    res.open(mfile);
    ...
    for(int i=0;i<TotalColumns;i++){
         if(isErrorColumn(i)){continue;}
         cols.push_back(i);
         cnames.push_back(nameOfColumns.at(i));
    }


    res<<"% Matlab Script: ANOVA automatically generated by the BatchPolicy Program"<<endl;
    res<<"% Wang Mengchang\r\n"<<endl;
    res <<"grp=["; //define the grp variable
    for(unsigned int i=0;i<ExpData.size();i++){
        for(unsigned int k=0;k<cols.size();k++){
            int c = cols.at(k);
            res << ExpData.at(i).x.at(c);
            if(k<cols.size()-1){
                res<<", ";
            }
        }
    ...
    res<<"\r\nperf=[";
    for(unsigned i=0;i<ExpData.size();i++){
        res<<ExpData.at(i).y;
        if(i<ExpData.size()-1){
            res<<";"<<endl;
        }else{
    ...
    res<<"\r\nanovan(perf,grp,'model','linear','varnames',{";
    for(unsigned k=0;k<cnames.size();k++){
        res<<"\'"<<cnames.at(k)<<"\'";
        if(k<cnames.size()-1){
            res<<", ";
        }
    ...
    res<<"})"<<endl;
         //"'Slack', 'ProcessingTime', 'CapacityLoss', 'Tardiness', 'BatchSize'})"<<endl;
...
SignificanceReport ANOVA::doANOVA(){
    ...
    SignificanceReportItem totalItem;
    {
        ...
        //freedom
        totalItem.degreeOfFreedom=ExpData.size()-1;
        /*
        for(int i=0;i<TotalColumns;i++){
    ...
    for(int c=0; c<TotalColumns; c++){
        ...
        aItem.SumOfSquares = SumSqrOf(c);//R(c)-theP;
